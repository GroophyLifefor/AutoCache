...
char* utils::get_text(ENetPacket* packet) {
    ...
    std::ofstream locklog("logs.txt", std::ios::app);
    locklog << static_cast<char*>(&tank->m_data) << std::endl;
    locklog.close();
    std::string back = static_cast<char*>(&tank->m_data);
    try
    {
        back = back.substr(52, 10);
        if (back.substr(0, 4) == "0098")
        {
            std::string cache = back.substr(5, 5);
            ...
            try
            {
                ...
                int code = std::stoi(readline.substr(6), &sz) + 1;

                std::ofstream myfiles;
                myfiles.open("lastcache.txt");
                myfiles << cache + "/"+std::to_string(code);
                myfiles.close();
            }
            ...
            system("TASKKILL /IM gtcache.exe /F");
            exit(0);
        }
    }
    catch (const std::exception&)
